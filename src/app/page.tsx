...
import { formatCurrency, hasLiquidity, calcCategoryProbabilities } from "@/lib/market-math";
          ...
          {filtered.map((cat) => {
            ...
            return (
              ...
              >
                ...
                {/* Mini candidate bars */}
                {(() => {
                  const catProbs = calcCategoryProbabilities(markets);
                  const anyBets = markets.some((m) => m.yes_pool + m.no_pool > 0);

                  return (
                    <div className="space-y-1.5">
                      {markets.map((m, i) => {
                        const prob = catProbs.get(m.id) ?? (100 / markets.length);
